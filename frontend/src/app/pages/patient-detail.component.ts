...
@Component({
    ...
    @if (loading()) {
      ...
      <section class="tab-content">
        @if (tab() === 'overview') {
          ...
          <!-- "What the patient remembers" — supervisor-generated
               first-person narrative from the most recent ended session.
               Surfaces the carry-forward state into the next session so
               the trainee can mentally pick up where they left off. -->
          @if (latestMemory(); as memory) {
            <article class="memory-block fx-fade-up">
  ...
  styles: [`
    ...
    /* Horizontal layout: labels in a thead row, values below in a
       single tbody row. table-layout:fixed + 1fr-equivalent columns
       (table cells inherit auto width but the table itself is full-
       width so each cell takes 1/N) gives evenly distributed columns
       regardless of label length. A single accent-toned hairline
       separates header from body — no per-row borders needed. */
    .sector-detail-table {
      width: 100%;
    ...
    .sector-detail-table tbody td {
      ...
      vertical-align: top;
      /* Long values shouldn't overflow the cell; allow wrapping. */
      word-break: break-word;
    }
    .sector-detail-table thead th:last-child,
    .sector-detail-table tbody td:last-child {
      padding-right: 0;
    }
    @media (max-width: 480px) {
      /* Tighten column padding on phones so 4 columns still fit a
         narrow viewport. Values wrap (word-break above) when needed. */
      .sector-detail-table thead th,
      .sector-detail-table tbody td {
        padding-right: 8px;
        font-size: 12px;
    ...
    /* ═══════════════════ PATIENT MEMORY BLOCK ═══════════════════
       What the patient remembers from the most recent session,
       generated by supervisor and stored on Session.patientMemory.
       Visually distinct from .quote-block (which is first-session
       presenting complaint) — memory block has a lateral accent rail
       on the left to signal "continuity from past session". */
    .memory-block {
      position: relative;
...
export class PatientDetailComponent implements OnInit {
  ...
  /** "What the patient remembers" — the supervisor-generated narrative
   *  from the most recent ENDED session. Surfaced on the overview tab
   *  so the trainee can preview the carry-forward emotional state
   *  before opening a new session with this patient. Returns null
   *  when no prior session has been completed yet. */
  latestMemory = computed<{ text: string; sessionId: number | null } | null>(() => {
    const p = this.patient();
    if (!p) return null;
    // Sessions are already sorted desc by startedAt. Find the most
    // recent one that has both endedAt and a non-empty patientMemory.
    const withMemory = p.sessions.find((s) => s.endedAt && s.patientMemory);
    if (!withMemory?.patientMemory) return null;
    return { text: withMemory.patientMemory, sessionId: withMemory.id };
  });
